<ul class="icons-list">
    @ability('superadministrador,administrador', 'ver-servidores')
         @if(in_admin_group())
            <li><a href="{{ route('comissionados.servidores.show',['id' => $servidor->id]) }}"
    ...
    @permission('atualizar-servidores')
        @if(in_admin_group())
        <li><a href="{{ route('comissionados.servidores.edit',['id' => $servidor->id]) }}"
               data-popup="tooltip" title="Editar" data-placement="bottom"><i
                        class="icon-pencil7"></i></a>
        </li>
        @elseif(auth()->user()->servidor_id == $servidor->id && $servidor->validado == 'N')
        <li><a href="{{ route('comissionados.servidores.edit',['id' => $servidor->id]) }}"
               data-popup="tooltip" title="Editar" data-placement="bottom"><i
                        class="icon-pencil7"></i></a>
        </li>
